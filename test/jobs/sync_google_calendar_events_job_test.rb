  ...
  test "sync calendar (同期の対象が送られてきたとき)" do
    ...
    event_list_response_body = {
      ...
      items: [
        ...
        {
          id: "2", status: "confirmed", summary: "r test2", start: { dateTime: Time.zone.local(2021, 1, 1, 0, 0, 0).iso8601 }, end: { dateTime: Time.zone.local(2021, 1, 1, 1, 0, 0).iso8601 }
        }
      ]
    }.to_json
    ...
    stub_request(:patch, "https://www.googleapis.com/calendar/v3/calendars/dummy_calendar_id/events/2").
    with { |request |
      body = JSON.parse(request.body)
      assert {  body == { "summary"=>"r test2 🔄" } }
    }.
    ...
    assert { calendar.google_calendar_events.sole.event_id == "2" }
    assert { calendar.google_calendar_events.sole.summary == "r test2 🔄" }
  end
end
